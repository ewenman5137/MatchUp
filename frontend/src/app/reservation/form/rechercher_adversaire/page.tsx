...
export default function RechercherAdversairePage() {
  ...
  const [heuresBloquees, setHeuresBloquees] = useState<string[]>([]);
  ...
  const fetchHeuresBloquees = async (sport: string, date: string) => {
    if (!sport || !date) return;
    const promises = Array.from({ length: 14 }, (_, i) => {
      const heure = (i + 8).toString().padStart(2, "0") + ":00";
      return fetch(`http://localhost:5000/api/blocage?sport=${sport}&date=${date}&heure=${heure}`)
        .then((res) => res.json())
        .then((data) => (data.bloque ? heure : null))
        .catch(() => null);
    });
    const results = await Promise.all(promises);
    setHeuresBloquees(results.filter((h): h is string => h !== null));
  };
  ...
  useEffect(() => {
    if (form.date && form.sport) {
      fetchHeuresBloquees(form.sport, form.date);
    }
          ...
          <div className="bg-gray-50 p-4 rounded shadow mb-10">
            ...
            <div className="grid grid-cols-2 gap-4 mb-4">
              <div>
                ...
                <input type="email" value={form.email} onChange={(e) => setForm({ ...form, email: e.target.value })} className="border w-full px-3 py-2 rounded mt-1" />
              </div>
              <div>
                <label className="text-sm font-medium">Sport</label>
                <select value={form.sport} onChange={(e) => setForm({ ...form, sport: e.target.value })} className="border w-full px-3 py-2 rounded mt-1">
              ...
              <div>
                ...
                <input type="date" value={form.date} onChange={(e) => setForm({ ...form, date: e.target.value })} className="border w-full px-3 py-2 rounded mt-1" />
              </div>
              <div>
                <label className="text-sm font-medium">Heure</label>
                <select value={form.heure} onChange={(e) => setForm({ ...form, heure: e.target.value })} className="border w-full px-3 py-2 rounded mt-1">
                  <option value="">-- Choisir une heure --</option>
                  {Array.from({ length: 14 }, (_, i) => {
                    const hour = i + 8;
                    const formatted = hour.toString().padStart(2, "0") + ":00";
                    const isBloquee = heuresBloquees.includes(formatted);
                    return (
                      <option key={hour} value={formatted} disabled={isBloquee}>
                        {formatted} {isBloquee ? "- bloqué" : ""}
                      </option>
                    );
                  })}
                </select>
              ...
              <div>
                ...
                <select value={form.niveau} onChange={(e) => setForm({ ...form, niveau: e.target.value })} className="border w-full px-3 py-2 rounded mt-1">
              ...
              <div>
                ...
                <input type="number" min={1} max={4} value={form.duree} onChange={(e) => setForm({ ...form, duree: Number(e.target.value) })} className="border w-full px-3 py-2 rounded mt-1" />
              </div>
            </div>
            <div className="mb-4">
              <label className="text-sm font-medium">Commentaire</label>
              <textarea value={form.commentaire} onChange={(e) => setForm({ ...form, commentaire: e.target.value })} className="w-full border px-3 py-2 rounded mt-1"></textarea>
            </div>
            <button className="bg-[#7A874C] text-white px-4 py-2 rounded" onClick={proposerRencontre}>Envoyer la proposition</button>
          </div>
        )}
